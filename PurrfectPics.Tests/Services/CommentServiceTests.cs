...
namespace PurrfectPics.Tests.Services
{
    public class CommentServiceTests
    {
        ...
        public async Task AddCommentAsync_CreatesNewComment()
        {
            ...
            _mockRepo.Setup(x => x.AddAsync(It.IsAny<Review>()))
            ...
            _mockRepo.Verify(x => x.AddAsync(It.Is<Review>(c =>
        ...
        {
            // Arrange
            var comment = new Review { Id = 1, PostedById = "user1" };
        ...
        {
            // Arrange
            var comment = new Review { Id = 1, PostedById = "user2" };
            _mockRepo.Setup(x => x.GetByIdAsync(1))
                .ReturnsAsync(comment);
            _mockRepo.Setup(x => x.DeleteAsync(comment))
                .ReturnsAsync(true);
        ...
        {
            // Arrange
            var comment = new Review { Id = 1, PostedById = "user1", Content = "Old" };
            _mockRepo.Setup(x => x.GetByIdAsync(1))
                .ReturnsAsync(comment);
            _mockRepo.Setup(x => x.UpdateAsync(It.Is<Review>(c => c.Content == "New")))
                .ReturnsAsync(true);
            ...
            // Assert
            Assert.True(result);
            _mockRepo.Verify(x => x.UpdateAsync(It.Is<Review>(c => c.Content == "New")), Times.Once);
        }
        ...
        public async Task EditCommentAsync_ReturnsFalse_WhenUserNotOwner()
        {
            ...
            // Assert
            Assert.False(result);
            _mockRepo.Verify(x => x.UpdateAsync(It.IsAny<Review>()), Times.Never);
        }
        ...
        public async Task GetCommentsForImageAsync_ReturnsCommentsFromRepository()
        {
            ...
            {
                new Review { Id = 1, CatImageId = 1, Content = "Comment 1" },
                new Review { Id = 2, CatImageId = 1, Content = "Comment 2" }
            };
