...
namespace PurrfectPics.Tests.Services
{
    ...
    {
        private readonly Mock<IBookRepository> _mockCatImageRepo;
        private readonly Mock<ITagService> _mockTagService;
        private readonly CatImageService _catImageService;

        public CatImageServiceTests()
        {
            _mockCatImageRepo = new Mock<IBookRepository>();
        ...
        {
            // Arrange
            var expectedImages = new List<Book>
    {
        new Book { Id = 1, Title = "Test Image 1" },
        new Book { Id = 2, Title = "Test Image 2" }
    };
        ...
        {
            // Arrange
            var expectedImage = new Book { Id = 1, Title = "Test Image" };
            ...
            var expectedImages = new List<Book>
    {
        ...
        {
            // Arrange
            var newImage = new Book { Title = "New Image" };
            var tags = new List<string> { "cute", "fluffy" };

            _mockTagService.Setup(ts => ts.EnsureTagExistsAsync("cute"))
                .ReturnsAsync(new Genre { Id = 1, Name = "cute" });
            _mockTagService.Setup(ts => ts.EnsureTagExistsAsync("fluffy"))
                .ReturnsAsync(new Genre { Id = 2, Name = "fluffy" });

            _mockCatImageRepo.Setup(repo => repo.AddAsync(It.IsAny<Book>()))
                .Returns(Task.CompletedTask)
                .Callback<Book>(img => img.Id = 1);
            ...
            _mockCatImageRepo.Verify(repo => repo.AddAsync(It.IsAny<Book>()), Times.Once);
        }
        ...
        {
            // Arrange
            var existingImage = new Book
            {
                Id = 1,
                Comments = new List<Review>(),
                Votes = new List<Rating>(),
                Favorites = new List<Wishlist>()
            };
        ...
        public async Task DeleteImageAsync_ClearsRelatedEntities_BeforeDeletion()
        {
            ...
            {
                Id = 1,
                Comments = new List<Review> { new Review() },
                Votes = new List<Rating> { new Rating() },
                Favorites = new List<Wishlist> { new Wishlist() }
            };
        ...
        {
            // Arrange
            var existingImage = new Book { Id = 1 };
            _mockCatImageRepo.Setup(repo => repo.GetByIdWithDetailsAsync(1))
                .ReturnsAsync(existingImage);
            _mockCatImageRepo.Setup(repo => repo.DeleteAsync(1))
        ...
        {
            // Arrange
            var recentImages = new List<Book>
    {
        new Book { Id = 1, Title = "Recent 1" },
        new Book { Id = 2, Title = "Recent 2" }
    };
            ...
            var expectedResults = new List<Book>
    {
        ...
        public async Task GetRecentUserImagesAsync_ReturnsUserImagesOrderedByDate()
        {
            // Arrange
            var userId = "user123";
            var userImages = new List<Book>
    {
        new Book { Id = 1, UploadDate = DateTime.UtcNow.AddDays(-1) },
        new Book { Id = 2, UploadDate = DateTime.UtcNow }
    };
