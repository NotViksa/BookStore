...
namespace PurrfectPics.Tests.Services
{
    public class VoteServiceTests
    {
        ...
        public async Task SubmitVoteAsync_AddsNewUpvote_WhenNoExistingVote()
        {
            ...
            _mockRepo.Verify(x => x.AddVoteAsync(It.Is<Rating>(v =>
        ...
        {
            // Arrange
            var existingVote = new Rating { UserId = "user1", CatImageId = 1, IsUpvote = false };
            _mockRepo.Setup(x => x.GetVoteAsync("user1", 1))
        ...
        {
            // Arrange
            var existingVote = new Rating { UserId = "user1", CatImageId = 1, IsUpvote = true };
            _mockRepo.Setup(x => x.GetVoteAsync("user1", 1))
                .ReturnsAsync(existingVote);
            _mockRepo.Setup(x => x.GetImageScoreAsync(1))
                .ReturnsAsync(0);
        ...
        {
            // Arrange
            var vote = new Rating { IsUpvote = true };
            _mockRepo.Setup(x => x.GetVoteAsync("user1", 1))
                .ReturnsAsync(vote);
        ...
        {
            // Arrange
            var vote = new Rating { IsUpvote = false };
            _mockRepo.Setup(x => x.GetVoteAsync("user1", 1))
                .ReturnsAsync(vote);
