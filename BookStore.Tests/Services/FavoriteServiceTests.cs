...
namespace PurrfectPics.Tests.Services
{
    public class FavoriteServiceTests
    {
        ...
        public async Task ToggleFavoriteAsync_AddsFavorite_WhenNotExists()
        {
            ...
            // Assert
            Assert.True(result);
            _mockRepo.Verify(x => x.AddFavoriteAsync(It.Is<Wishlist>(f =>
                f.UserId == userId && f.CatImageId == imageId)), Times.Once);
            _mockRepo.Verify(x => x.RemoveFavoriteAsync(It.IsAny<Wishlist>()), Times.Never);
        }
        ...
        public async Task ToggleFavoriteAsync_RemovesFavorite_WhenExists()
        {
            ...
            var userId = "user1";
            var imageId = 1;
            var existingFavorite = new Wishlist { UserId = userId, CatImageId = imageId };
            _mockRepo.Setup(x => x.GetFavoriteAsync(userId, imageId))
            ...
            _mockRepo.Verify(x => x.AddFavoriteAsync(It.IsAny<Wishlist>()), Times.Never);
        }
        ...
        public async Task GetUserFavoritesAsync_ReturnsImagesFromRepository()
        {
            ...
            {
                new Book { Id = 1, Title = "Fluffy" },
                new Book { Id = 2, Title = "Whiskers" }
            };
        ...
        public async Task GetRecentFavoritesAsync_ReturnsOrderedFavorites()
        {
            ...
            var favorites = new List<Wishlist>
            {
                new Wishlist { FavoritedDate = now.AddDays(-2), CatImage = new Book { Id = 1 } },
                new Wishlist { FavoritedDate = now, CatImage = new Book { Id = 2 } },
                new Wishlist { FavoritedDate = now.AddDays(-1), CatImage = new Book { Id = 3 } }
            };
        ...
        public async Task GetRecentFavoritesAsync_ReturnsLimitedCount()
        {
            ...
            _mockRepo.Setup(x => x.GetRecentFavoritesAsync(userId, 2))
                .ReturnsAsync(new List<Wishlist> { new Wishlist(), new Wishlist() });
